CLASS zcl_aux_travel_del_0176 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    TYPES: tt_travel_reported  TYPE TABLE FOR REPORTED zi_travel_log_0176,
           tt_booking_reported TYPE TABLE FOR REPORTED zi_booking_0176,
           tt_bookspl_reported TYPE TABLE FOR REPORTED zi_bookspl_log_0176.
...
CLASS zcl_aux_travel_del_0176 IMPLEMENTATION.
  METHOD calculate_price.
    ...
    LOOP AT lt_read_booking INTO DATA(ls_booking) GROUP BY ls_booking-travel_id INTO DATA(lv_travel_key).
        ...
        LOOP AT GROUP lv_travel_key INTO DATA(ls_booking_result) GROUP BY ls_booking_result-currency_code
        INTO DATA(lv_currency_code).
            ...
            IF lv_currency_code EQ <fs_travel>-currency_code.
   ...
   LOOP AT lt_read_supplements INTO DATA(ls_supplement) GROUP BY ls_supplement-travel_id
   INTO lv_travel_key.
   ...
   ASSIGN lt_read_travel[ KEY entity COMPONENTS travel_id = lv_travel_key ] TO <fs_travel>.
    ...
    LOOP AT GROUP lv_travel_key INTO DATA(ls_supplements_result)
        GROUP BY ls_supplements_result-currency_code INTO lv_currency_code.

        lv_total_suppl_price = 0.

        LOOP AT GROUP lv_currency_code INTO DATA(ls_suppl_line).

            lv_total_suppl_price += ls_suppl_line-price.
   ...
   MODIFY  ENTITIES OF zi_travel_log_0176 ENTITY Travel
